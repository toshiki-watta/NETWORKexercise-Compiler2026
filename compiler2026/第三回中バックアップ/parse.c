...
#define MAXSYMS 32
// シンボルテーブルのエントリ構造体、同時に覚える識別子の最大長も定義
typedef struct {
    int addr;
    char v[MAXIDLEN+1];
} s_entry;
...
static int sym_install(const char *name){
    int idx = sym_lookup(name);
    if (idx != -1) return idx;
    if (s_count >= MAXSYMS) error("symbol table overflow");
    strcpy(s_table[s_count].v, name);
    s_table[s_count].addr = s_count; /* アドレスは 0,1,2,... */
    return s_count++;
}
...
static void emit_label(int lbl){
    fprintf(outfile, "L%d:\n", lbl);
}
